...
function SearchSession() {
  ...
  const ClickSearch = async () => {
    try {
      const respone = await axios.get(
        `http://localhost:8080/restaurant/search?word=${search}&page=${page}`
      );
  ...
  const items = restaurant.map((data) => {
    return (
      <div
        className="restaurant-list-area"
      ...
      >
        ...
        <div className="restaurant-btn-area">
          <button
            ...
            onClick={() => clickBookmark(data.id)}
          >
            즐겨찾기
          </button>
          <button className="restaurant-btn">
            <Link
              to={`/restaurant/${data.id}/review`}
  ...
  return (
    <div className="SearchSession">
      <div className="search-bar-area">
        ...
        <input type="serach" className="search-bar" onChange={getValue}></input>
        <img
      ...
      <div className="search-map-area">
        <Map
          restaurant={
            search ? restaurant : selectRest ? [selectRest] : restaurant
          }
          restLen={restLen}
          setLat={setLat}
          setLon={setLon}
      ...
      <div className="collect-area">
        <div className="collect-list">
          <button
            className={`list-btn ${
              btnSelected === "distance" ? "selected" : ""
            }`}
            onClick={() => handlePaginationTypeChange("distance")}
          >
            거리순
          </button>
          <button
            className={`list-btn ${btnSelected === "degree" ? "selected" : ""}`}
            onClick={() => handlePaginationTypeChange("degree")}
          >
            별점순
          </button>
          <button
            className={`list-btn ${
              btnSelected === "reviews" ? "selected" : ""
            }`}
            onClick={() => handlePaginationTypeChange("reviews")}
          >
            리뷰순
          </button>
        </div>
        <div className="collect-categori">
          <button
            className={`categori-btn ${
              btnSelected === "onlyrest" ? "selected" : ""
            }`}
            onClick={() => handlePaginationTypeChange("onlyrest")}
          >
            음식점
          </button>
          <button
            className={`categori-btn ${
              btnSelected === "onlycafe" ? "selected" : ""
            }`}
            onClick={() => handlePaginationTypeChange("onlycafe")}
          >
            카페
          </button>
        </div>
